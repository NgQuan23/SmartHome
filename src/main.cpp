...
void setup() {
  ...
  wifiInit();
  Serial.println("wifiInit DONE");
  mqttInit();
  Serial.println("mqttInit DONE");

  lcd.setCursor(0, 1);
  lcd.print("Init Storage... ");
  storageInit();
  Serial.println("storageInit DONE");
...
void loop() {
  ...
  // Initialize network-dependent services only after background WiFi connects
  if (!networkServicesInitialized && WiFi.status() == WL_CONNECTED) {
    networkServicesInitialized = true;
    Serial.println("Network connected, initializing network services...");

    lcd.setCursor(0, 1);
    lcd.print("Init Firebase...");
    firebaseInit();
    Serial.println("firebaseInit DONE");
...
void readSensors() {
  ...
  if (gasWarning <= 0)
    gasWarning = GAS_LEVEL_2;

  int distanceLevel = 0;
  if (distance >= 0) {
    if (distance <= DIST_LEVEL_3)
      distanceLevel = 3;
    else if (distance <= distCritical)
      distanceLevel = 2;
  ...
  if (distance > 0 && distance <= DIST_LEVEL_3) {
    ...
    if ((nowAlert - lastAlertTime) >= ALERT_COOLDOWN_MS) {
      lastAlertTime = nowAlert;
      String msg =
          String("Warning: Water level is high (") + (int)distance + "cm)";
      telegramSend(msg);
    }
  }
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (distance < 0)
      ...
      String distStr =
          String((int)distance) + "/" + String(distCritical) + "cm";
      lcd.print(distStr);
